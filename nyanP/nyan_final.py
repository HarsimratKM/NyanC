#Author's Name: Excelia Dewi and harsimrat Kaur
#Last Modified By: Excelia Dewi and harsimrat Kaur
...
""" 
  ...
  Version 1.0 - *     complete the easy level, the end and start screen not working properly 
                      and background music need to be fixed
  ...
  Version 3.0 - *     hard level completed. burger sprites have been added and the function is to give
                      user extra 50 points for score everytime user can collect it. 
                      More enemies, tac nyan, small and huge asteroids.
                      problem still persist in start and end screen also the music background. 
                        
  Version 4.0 - *     the music background fixed.
    """
...
class NyanCat(pygame.sprite.Sprite):
    ...
    def update(self):
        #make Nyan Cat move cannot exceed the top and the bottom of the screen
...
#creating burger class that gives +50 pointsupon collision
class Burger(pygame.sprite.Sprite):
    def __init__(self):
        pygame.sprite.Sprite.__init__(self)
        self.image = pygame.image.load("burger.gif")
        self.image = self.image.convert()
        self.rect = self.image.get_rect()
        self.reset()
...
#create bonus class that grants +1 Life upon collision       
...
def gameHard():
    ...
    #handle multiple sprites and the order each sprites get update
    friendSprites = pygame.sprite.OrderedUpdates(space, baloon, lives, burger, nyan)
    
    #create multiple Tac Nyan sprites
    enemySprites = pygame.sprite.Group(asteroid1,asteroid2,asteroid3, bigAsteroid1, bigAsteroid2, 
                                       bigAsteroid3, tac1, tac2, tac3)
    ...
    while keepGoing:
        ...
        #check collisions with lives
        if nyan.rect.colliderect(lives.rect):
            nyan.sndGain.play()
            lives.reset()
            scoreboard.lives += 1
        
        #check collisions with nyanBalloon
        if nyan.rect.colliderect(baloon.rect):
            nyan.sndGain.play()
            baloon.reset()
            scoreboard.score += 100
        ...
        #check collisions with nyanBalloon
        if nyan.rect.colliderect(burger.rect):
            nyan.sndGain.play()
            burger.reset()
            scoreboard.score += 50
            ...
            if event.type == pygame.QUIT:
                ...
                donePlaying = True
                 ...
                 if event.key == pygame.K_1:
                     #play the game in easy mode when th user presses 1
                    score = gameEasy()
                 if event.key == pygame.K_2:
                     #play the game in medium difficulty mode when user presses 2
                    score = gameMedium()
                 if event.key == pygame.K_3:
                     #play the game in hard mode when the user presses 3
                    score = gameHard()
